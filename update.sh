...
# 调整部分DNS相关应用在LuCI界面中的菜单分类位置（如需调整可取消注释）
update_dns_app_menu_location() {
    # 将 smartdns 菜单移动到“DNS”分类
